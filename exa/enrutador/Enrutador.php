...
namespace exa\enrutador;

use exa\modelo\RESTArticulo;
use Exception;
  ...
  private function idenPeticion()
  {
    try {
      ...
      if (
        ...
        throw new \Exception("peticion no valida", 1);
      }
    } catch (\Exception $e) {
      $this->enviaError($e);
    }
  ...
  private function procPeticion()
  {
    try {
      ...
      if (!class_exists($claseModelo) || !method_exists($claseModelo, $metodo)) {
        throw new \Exception("Clase $claseModelo o metodo $metodo no existe", 2);
      }
